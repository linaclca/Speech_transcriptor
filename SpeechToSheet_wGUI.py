...
def midi_to_png(midi_path, output_png):
  ...
  if midi_path == 'output/speech_to_drums.mid':
    score.metadata.title = 'Drum notation'
  else:
    score.metadata.title = 'Melody notation'

  score.metadata.composer = 'you'
...
class VoiceRecorder:
  ...
  def show_sheet_music(self):
    ...
    if not (drum_success and melody_success):
        messagebox.showerror("Conversion Error", "Error converting MIDI to sheet music.")
        return
    
    drum_png = 'output/sheet_drum-1.png'
    melody_png = 'output/sheet_melody-1.png'

    # Check if PNGs exist
    if not os.path.exists(drum_png):
        print(f"File not found: {drum_png}")
        messagebox.showerror("Error", f"Drum sheet music image not found: {drum_png}")
        return
    if not os.path.exists(melody_png):
        print(f"File not found: {melody_png}")
        messagebox.showerror("Error", f"Melody sheet music image not found: {melody_png}")
        return

    if os.path.exists(drum_png):
      try:
        img = Image.open(drum_png)
        img.show()  # Opens in default viewer, ensuring it's a valid image
      except Exception as e:
        print(f"Error opening drum image: {e}")
    else:
      print(f"File not found: {drum_png}")

    if os.path.exists(melody_png):
      try:
        img = Image.open(melody_png)
        img.show()
      except Exception as e:
        print(f"Error opening melody image: {e}")
    else:
      print(f"File not found: {melody_png}")
    ...
    #try:
    #    drum_img = Image.open(drum_png).convert("RGB")
    #    melody_img = Image.open(melody_png).convert("RGB")
    #except Exception as e:
